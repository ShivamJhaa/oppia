...
describe('Create New Chapter Modal Controller', function() {
    ...
    function() {
      expect(this.nodeTitles).toEqual(nodeTitles);
      expect(this.errorMsg).toBe(null);
    });

  it('should validate explorationId correctly',
    function() {
      this.explorationId = 'validId';
      expect(this.validateExplorationId()).toBeTrue();
      this.explorationId = 'oppia.org/validId';
      expect(this.validateExplorationId()).toBeFalse();
    });
  ...
  it('should update thumbnail filename when changing thumbnail file',
    function() {
      ...
      this.updateThumbnailFilename('abc');
      expect(storyUpdateSpy).toHaveBeenCalled();
      expect(this.editableThumbnailFilename).toEqual('abc');
    });
  ...
  it('should update thumbnail bg color when changing thumbnail color',
    function() {
      ...
      this.updateThumbnailBgColor('abc');
      expect(storyUpdateSpy).toHaveBeenCalled();
      expect(this.editableThumbnailBgColor).toEqual('abc');
    });
  ...
  it('should check if chapter is valid when it has title, exploration id and' +
    ...
    expect(this.isValid()).toEqual(false);
    this.title = 'title';
    this.explorationId = '1';
    expect(this.isValid()).toEqual(false);
    this.editableThumbnailFilename = '1';
    expect(this.isValid()).toEqual(true);
    this.explorationId = '';
    expect(this.isValid()).toEqual(false);
  });
...
//   });
  ...
  function() {
    this.explorationId = 'exp_1';
    this.updateExplorationId();
    expect(this.invalidExpErrorString).toEqual(
...
//     });
  ...
  it('should not save when the chapter title is already used', function() {
    this.title = nodeTitles[0];
    this.save();
    expect(this.errorMsg).toBe('A chapter with this title already exists');
  ...
  it('should clear error message when changing exploration id', function() {
    ...
    this.resetErrorMsg();
    expect(this.errorMsg).toBe(null);
    expect(this.invalidExpId).toBe(false);
    expect(this.invalidExpErrorString).toBe(
